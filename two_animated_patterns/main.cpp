...
const char *WINDOWTITLE = { "OpenGL / Project#5 -- Panha An" };
...
int        WhichShader;            // NTEX, WTEX, WDIST
int        Xmouse, Ymouse;            // mouse values
float      Xrot, Yrot;                // rotation angles in degrees

bool VSC, FSC;                      // Vertext / Fragment Shader Changes
...
Display( )
{
    ...
    if ( WhichShader == NS )
    {
        VSC = false;
        FSC = false;
    } else if ( WhichShader == VS ) {
        VSC = true;
        FSC = false;
    } else if ( WhichShader == FS ) {
        VSC = false;
        FSC = true;
    } else {
        FSC = true;
    ...
    S0 = .1; T0 = .35; uSize = 0.15;
    Uka = 0.75; Ukd = 0.6; Uks = 0.15;
    ...
    Pattern->SetUniformVariable("uSpecularColor", USColorR, USColorG, USColorB);
    Pattern->SetUniformVariable("uKa", Uka);
    Pattern->SetUniformVariable("uKd", Ukd);
    Pattern->SetUniformVariable("uKs", Uks);
    Pattern->SetUniformVariable("uVS", VSC);
    Pattern->SetUniformVariable("uFS", FSC);
    OsuSphere( RADIOUS, SLICES, STACKS );
    Pattern->UnUse( );
...
InitMenus( )
{
    ...
    glutAddMenuEntry( "No Shader Changes", NS );
    glutAddMenuEntry( "Vertex Shader Changes Only", VS);
    glutAddMenuEntry( "Fragment Shader Changes Only", FS);
    glutAddMenuEntry( "Vertex + Fragment Shader Changes", VFS);
